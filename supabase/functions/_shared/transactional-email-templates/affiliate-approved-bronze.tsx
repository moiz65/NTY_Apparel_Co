...
const Email = ({ first_name, code, link, commission_percent, discount_percent }: Props) => {
  ...
  const commission = commission_percent ?? 10
  const discount = discount_percent ?? 10
  const affLink = link || `${SITE}/?ref=${code || 'YOURCODE'}`
  return (
    <Html lang="en" dir="ltr">
      <Head />
      <Preview>Welcome to Bronze — Your Natty Apparel Affiliate Account Is Live</Preview>
      <Body style={main}>
        <Container style={container}>
          <Heading style={brand}>NATTY APPAREL</Heading>
          <Text style={tierBadge}>TIER · BRONZE</Text>
          <Heading style={h1}>You're In — Welcome to Bronze</Heading>
          <Text style={text}>Hey {name},</Text>
          <Text style={text}>
            Your affiliate application has been approved. You're starting at <strong>Bronze · {commission}% commission</strong>,
            and your code gives customers <strong>{discount}% off</strong>.
          </Text>
          ...
          <Text style={bullet}>• Partner portal & program details: <a href={`${SITE}/partners`} style={linkStyle}>{SITE}/partners</a></Text>
          <Text style={bullet}>• Shop the drop: <a href={`${SITE}/shop`} style={linkStyle}>{SITE}/shop</a></Text>
          <Text style={bullet}>• Support: <a href="mailto:support@ntyapparel.com" style={linkStyle}>support@ntyapparel.com</a></Text>

          <Text style={h2}>How to climb</Text>
          <Text style={bullet}>• Hit $5,000 GMV (30-day rolling) to unlock <strong>Silver · 15%</strong></Text>
          <Text style={bullet}>• Hit $10,000+ GMV to unlock <strong>Gold · 20%</strong></Text>

          <Text style={text}>Track every click, sale, and commission in real time from your dashboard.</Text>
...
const tierBadge = { fontSize: '11px', letterSpacing: '0.3em', color: '#7a4a1a', backgroundColor: '#f3e3cf', padding: '6px 10px', display: 'inline-block', margin: '0 0 16px', fontWeight: 700 }
